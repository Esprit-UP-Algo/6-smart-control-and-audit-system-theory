...
query.prepare("INSERT INTO MISSION (CODE_MISSION, DATE_MISSION, STATUT_MISSION,CODE_ENTREPRISE)"
              "VALUES (:code_mission , :date_mission, :statut_mission,:stqt)");
query.bindValue(":code_mission",res);
query.bindValue(":date_mission",date_mission);
query.bindValue(":statut_mission",statut_mission);
query.bindValue(":stqt",stqt);
...
QSqlQueryModel * mission::tableau(){
    QSqlQueryModel * model=new QSqlQueryModel();
    model->setQuery("select * from MISSION WHERE statut_mission= 'Done'");
