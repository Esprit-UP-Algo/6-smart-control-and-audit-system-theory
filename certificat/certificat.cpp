...
{
    QSqlQuery query;
    QString idCertificatStr = QString::number(idcertificat);
    QString resultatAuditStr = QString::number(resultau);
    QString dureeCertificatStr = QString::number(durecertif);

    query.prepare("UPDATE certificat SET RESULTATDAUDIT = :resultatdaudit, DURECERTIFICAT = :durecertificat, OBJECTIFCERTIFICAT = :objectifcertificat WHERE IDCERTIFICAT = :idcertificat");
    query.bindValue(":idcertificat", idCertificatStr);
    query.bindValue(":resultatdaudit", resultatAuditStr);
    query.bindValue(":durecertificat", dureeCertificatStr);
    query.bindValue(":objectifcertificat", objaudit);
